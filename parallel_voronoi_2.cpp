...
deque<Cell> Voronoi(Cell box, deque<Point> listpoints,float boxsize)
{
    ...
    // Início da paralelização com OpenMP para o primeiro loop
#pragma omp parallel for
    for (int i = 0; i < listpoints.size(); i++)
    {
        Point p = listpoints[i]; // Seleciona o ponto atual para ser o centro da célula.
        Cell local_cell = cell;  // Cria uma cópia local da célula base para evitar conflitos de dados entre threads.
...
#pragma omp parallel for
        for (int j = 0; j < listpoints.size(); j++)
        {
            ...
            if (p.x != q.x || p.y != q.y) // Verifica se os pontos p e q não são os mesmos.
            {
        ...
        // Seção crítica para garantir que apenas uma thread por vez possa executar o bloco de código a seguir.
        // Isso é necessário porque o deque `cells` está sendo modificado.
#pragma omp critical
        cells.push_back(local_cell); // Adiciona a célula local processada ao deque de células.
    }
